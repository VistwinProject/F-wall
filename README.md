...
三層高斯疊出「細亮芯 + 貼身內暈 + 大範圍體積外暈」，參數在 `config/frame.js` 的 `GLOW`：

| 層 | stdDeviation | alpha 放大 | 峰值 | 角色 |
| --- | --- | --- | --- | --- |
| `near` | 2 | 1.0 | 29% | 貼著核心 |
| `mid` | 7 | 2.0 | 17% | 中距離 |
| `halo` | 22 | 3.2 | 9% | 體積外暈 |

橫向衰減（距中心 0/4/8/15/25/40 → 46%/25%/16%/8.5%/4.6%/1.7%），可見半徑約 ±38 單位。
最後把未模糊的 `SourceGraphic` 疊回最上面，亮芯保持銳利。
...
`feComponentTransfer` 的 gain 是把高斯攤掉的亮度補回來 —— 1.5 寬的線用 σ=22 模糊後峰值只剩 2.7%。

濾鏡區域用整個 viewBox。σ=22 理論上要 3σ=66 餘裕，而大框上下只有 47.5，數學尾巴會被切一點，
但可見半徑 38 < 47.5 所以看得見的部分沒被裁。**`halo.sd` 調超過約 15 之後上下邊就會開始截斷。**

效能：一個濾鏡三次高斯 ≈ 3.0 倍畫布面積，但框架完全靜態（`LineFrame` 不吃 prop），
