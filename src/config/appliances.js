...
export const COLORS = {
  ...
  highlight: '#e8f8ff', // 最亮交會點（實測 #dcf6fe / #effcfd）；也當背景光帶的亮芯

  // ── 光束階（beam）——只給背景光帶用，實測自 /style 的 ANLB 影片 ──
  // 影片的光帶不是「同一色調疊不同透明度」，而是沿半徑做色相位移：
  //   亮芯近白 → 芯外 8 單位就跳到高飽和純藍 → 尾端收成暗藍，±124 單位才沉回背景。
  // 這是 additive bloom 的指紋，單色多層描邊做不出來，所以每層必須各自帶顏色。
  // 這一階刻意比 accent 飽和一階：accent 是 UI 用色，光帶是「光源」，用途不同。
  // 色相仍鎖在 200–228°（205.6 / 217.3 / 221.0 / 223.6）。
  beamHot: '#0092ff', // 亮芯外緣，影片在此已是高飽和純藍（實測 #0083fc / #0092ff）
  beamMid: '#0052d9', // 中段（實測 #0168f5 → #003598 之間）
  beamFar: '#002575', // 外暈（實測 #011d63）
  beamTail: '#001858', // 最外緣：振幅只剩 37/255，只在暗角看得出來
