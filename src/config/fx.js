...
export const FX = {
  ...
  beam: {
    ...
    tailLen: 0.16, // 拖尾長度（佔整條路徑的比例）。指數衰減，實際看得到的約 3 倍這個值

    base: 0.16, // 導線底光：active 時整條線都看得到一點
    peak: 1.1, // 彗星身部的亮度
  ...
  frame: {
    ...
    soft: 0.1, // 外圍柔光
    blockIdle: 0.55, // 家電框外圈 idle 時的亮度倍率
    blockOn: 1.8, // active 時的亮度倍率
  },

  // ── 微粒 ──────────────────────────────────────────────────────────────────
  sparks: {
    max: 420, // 環狀緩衝容量（九條線共用）
    every: 0.035, // 每條線每隔幾秒吐一顆
    life: 0.9, // 壽命（秒）
    size: 5.5, // 起始大小（世界單位）
    spread: 7, // 相對彗星頭的隨機偏移半徑
    bright: 1.6, // 亮度
  },
