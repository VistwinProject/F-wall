...
export const FX = {
  ...
  frame: {
    ...
    width: 27, // ribbon 總寬。光暈的最大擴散半徑就是 width/2，要更寬的暈就加這個
    // ⚠ coreSharp 是「相對於半寬」的指數，所以 width 改了就要跟著改，
    //   否則亮芯會跟著變粗。26 寬配 17 ≈ 亮芯 1 單位（與舊的 14 寬配 9 相同）；
    //   27 只差 3.8%，芯跟著粗 3.8%，看不出來，所以 coreSharp 維持 17。
    coreSharp: 17.0,
    softSharp: 2.2, // 外圍柔光的指數。越小暈越開、越平
    // 亮芯亮度（> bloom threshold 才會泛光）。
    // ⚠ 芯層與暈層拆開之後，芯的峰值不再吃到 soft 那一份（芯是 MAX、暈是加法），
    //   所以這個值要比拆開前高一些才會等亮。拆開前 1.35 → 峰值 626。
    core: 1.95,
    soft: 0.3, // 外圍柔光亮度 —— 這一項就是「光暈」的主體
    // 背景框架自己的呼吸，與家電／核心那組（1.5 秒）分開。
    breathe: { period: 3.0, lo: 0.55 },
  ...
  // 這個開關同時管九個家電框、核心框、電視預留區 —— 關掉之後 SVG 那層完全沒有白框，
  // 所有輪廓都來自 canvas 的發光外圈。
